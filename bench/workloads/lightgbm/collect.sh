...
#
# Collect MLP training features from a LightGBM run.
#
...
OUT="${OUT:-$XTIER_RESULTS/features_lightgbm.csv}"
# LightGBM touches memory far more sparsely than the tree workloads, so it
# wants a longer sampling period; too short and the NMI cost dominates.
PEBS_PERIOD="${PEBS_PERIOD:-30000}"
EPOCH_MS="${EPOCH_MS:-100}"

export N_ROWS="${N_ROWS:-7000000}"
export N_FEATURES="${N_FEATURES:-400}"
export N_ROUNDS="${N_ROUNDS:-200}"
...
echo "[*] lightgbm: ${N_ROWS}x${N_FEATURES}, ${N_ROUNDS} rounds, ${SECONDS_TO_RUN}s"
numactl --cpunodebind=0 --preferred=1 \
    python3 "$HERE/lightgbm_bench.py" \
...
echo "    train with: XTIER_OUTPUT_HEADER=ml/models/mlp_q8_lightgbm.h \\"
